...
export default function CreateReservation() {
  ...
  const bookTable = async (tableId) => {
    ...
    try {
      await api.post('/reservations', { tableId, date, timeSlot, guests: parseInt(guests) });
      setSuccess('Table booked successfully! 🎉');

      // refresh the list so the booked table disappears
      const { data } = await api.get('/reservations/availability', {
        params: { date, timeSlot, guests }
      });
      setTables(data.availableTables);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-700 mb-4">Find Available Tables</h2>
        <form onSubmit={searchTables} className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label htmlFor="date" className="block text-sm font-medium text-gray-700 mb-1">Date</label>
            <input id="date" type="date" value={date} min={today}
              onChange={e => { setDate(e.target.value); setSearched(false); }}
              required className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>

          <div>
            <label htmlFor="timeSlot" className="block text-sm font-medium text-gray-700 mb-1">Time Slot</label>
            <select id="timeSlot" value={timeSlot}
          ...
          <div>
            ...
            <input id="guests" type="number" value={guests} min="1" max="20"
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          {tables.length === 0 ? (
            <p className="text-gray-500 text-center py-4">
              No tables available for this slot. Try a different time or date.
            </p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {tables.map(table => (
                <div key={table._id} className="border border-gray-200 rounded-lg p-4 flex flex-col items-center">
                  <div className="text-3xl mb-2">🪑</div>
                  <h3 className="text-lg font-semibold text-gray-800">Table {table.tableNumber}</h3>
                  <p className="text-sm text-gray-500 mb-3">Seats {table.capacity}</p>
                  <button onClick={() => bookTable(table._id)} disabled={booking}
